...
class Metasploit3 < Msf::Auxiliary
	def initialize
		...
		register_options([
			OptPath.new('WORDLIST', [ false, "Source list",""]),
			OptString.new('WORDS', [ false, "Word(s) to leet",""]),
			OptPath.new('OUTPUTFILE', [ false, "Output to file",""])
		], self.class)
	end
	...
	def emit(level, buffer) 
		if level >= @max_level
			@output.print buffer + "\n" if @tofile 
			print_status buffer.to_s if @print
			return
		end
		@perm_array[level].each do |char|
	...
	def run()
		...
		@tofile = outputfile.length != 0
		@print = datastore['VERBOSE']
		...
		if word_list.length != 0
			...
			return print_error("#{word_list} not readable") unless File.readable? word_list
		...
		input.each do |word|
			...
			# Cache perm_array length
			@max_level = @perm_array.length
			
			# Recursively generate combinations
			emit(0,"")
		end
		
		@output.close if @tofile
	end
